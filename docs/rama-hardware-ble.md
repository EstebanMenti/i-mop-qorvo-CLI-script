...
| F10 | Verificación end-to-end contra hardware real (`validate`/`calibrate` completos, sesión TWR real, direccionamiento ADDR/PADDR) | **hecho parcialmente** (2026-08-13): sesión TWR real 50/50 SUCCESS y validación de comandos de lectura confirmadas; recalibración y direccionamiento **bloqueados por un hallazgo crítico** — ver [resultados-verificacion-ble.md](resultados-verificacion-ble.md) |
...
| Escritura de `RESPF`/`INITF` completo (~130+ caracteres) y de `CALKEY <clave> <valor>` | Necesario para calibración y para reconfigurar direccionamiento FiRa | **[CRÍTICO, sin resolver]** Confirmado con hardware real (F10, 2026-08-13) que **no es confiable**: `CALKEY <clave> <valor>` falló 0/5-6 intentos (timeout total, hasta con 30s de espera); `RESPF` con parámetros completos funcionó 1/7 veces (la sesión TWR exitosa de F10) y falló las 6 veces siguientes, incluso tras power-cycle físico del nRF52840. Descartado como causa: truncado en la escritura BLE (el log del cliente confirma el payload completo), `response=True` vs `False` (igual en ambos), y estado "atascado" recuperable (ningún reset lo arregló). Bloquea `dwm calibrate` y la reconfiguración de `ADDR`/`PADDR` — ver `docs/resultados-verificacion-ble.md` §3 para el detalle completo y la recomendación de investigar `qorvo_bridge.c` en el repo hermano |
