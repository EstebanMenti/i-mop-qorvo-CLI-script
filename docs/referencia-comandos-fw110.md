...
>
> **Persistencia confirmada:** repitiendo la prueba con `UART 1` **+ `SAVE`**, un ciclo de alimentación posterior **no** revirtió el cambio — la placa siguió sin responder por USB tras reiniciar. La configuración quedó grabada en NVM. Para revertirla hace falta acceso físico a los pines UART: `UART 0` (+ `SAVE` si se quiere persistir también la reversión) enviado desde ahí.
